...
sap.ui.define([
	...
	"ESM2/util/CharHeaderFactory",
	"ESM2/util/ODataManager",
	"ESM2/util/ODataManagerException",
	"ESM2/util/Util"
	...
	var PLMODataManager = function (oComponent) {
		...
		// EXT_CLASS
		oComponent.initClassExtension(this, "ESM2.util.PLMODataManager", arguments);
	...
	PLMODataManager.prototype.requestForUpdateUsagedat = function (oComposition) {
		...
		var fProcess = function (aBatchResponses) {
			var oInstanceKey = aBatchResponses[0].data;
			return oInstanceKey;
		};
		var oRequest = {
	...
	PLMODataManager.prototype.requestForFetchGenValHelp = function (oInstanceKey) {
		this._checkInit();

		// this._checkInstanceKey(oInstanceKey);

		var sParams = this._escapeParamsForFI({
			FIELDNAME: oInstanceKey.FIELDNAME,
			FIELDVAL: oInstanceKey.FIELDVALUE
		});

		var oBatchRequest = this._createBatchOperation("/SearchHelp?" + sParams, "GET");
		...
		var oRequest = {
			messageKey: "PLMODataManager.error.valuehelp.fetch",
			batchRequests: [oBatchRequest],
			process: fProcess
		};
	...
	PLMODataManager.prototype.requestForFetchLogo = function (Key) {
		...
		var aBatchRequest = [];
		var oBatchRequest = this._createBatchOperation("/GetSpecheadThumb?" + sParams, "GET");
		var oBatchRequest1 = this._createBatchOperation("/GetLogoDoc?", "GET");
		aBatchRequest.push(oBatchRequest);
		aBatchRequest.push(oBatchRequest1);

		var fProcess = function (aBatchResponses) {
			var logo = aBatchResponses[0].data;
			var logo1 = aBatchResponses[1].data.results[0];
			var logodat = [];
		...
		var oRequest = {
			...
			batchRequests: aBatchRequest,
			process: fProcess
		};

		return oRequest;
	};
	PLMODataManager.prototype.requestForUpdateLogo = function (Key) {
		...
		var oBatchRequest = this._createBatchOperation("/UpdateSpecheadThumb?" + sParams, "POST");
		...
		var oRequest = {
			messageKey: "PLMODataManager.error.logo.update",
			batchRequests: [oBatchRequest],
			process: fProcess,
			change: true
		};
	...
	PLMODataManager.prototype.requestForFetchRecipe = function (Key) {

		var sParams = this._escapeParamsForFI({
			SUBID: Key.SUBID
		});

		var oBatchRequest = this._createBatchOperation("/GetRcpFormula?" + sParams, "GET");

		var fProcess = function (aBatchResponses) {
			var rcptab = aBatchResponses[0].data.results;
			var oResponse = {
				entries: rcptab
			};
		...
		var oRequest = {
			messageKey: "PLMODataManager.error.rcp.fetch",
			batchRequests: [oBatchRequest],
			process: fProcess
		};
