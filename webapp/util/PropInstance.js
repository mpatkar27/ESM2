...
sap.ui.define([
	...
	"ESM2/util/UsageDiag",
	"ESM2/util/CharEditCommon",
	"ESM2/util/AddCountry",
	"sap/ui/model/Filter",
	"sap/ui/model/FilterOperator",
	"ESM2/util/LangValueHelp"
], function (assert, Label, MultiInput, Token, smartfield, SmartMultiInput, JSONModel, Column, ColumnListItem, TableManager, PropBase,
	ModelManagerAdmin,
	...
	var PropInstance = function (oComponent, oTreeNode, oCollection, bEditable, udtprop, compindref, mandatoryfieldcounter, udtinst, layout,
		secndrn, objlay, proplist, oPropInfo, propindex) { //FIXMEVC:if bEditable is false, then make this propcontrol readonly.
		PropBase.call(this, oComponent, "ESM2.frag.PropInstance");
		...
		var _onCharEdit = function (oCollection, oCharHeader, oEvent) { //FIXMEVC:refactor
			...
			this._progmod = oEvent.getSource().getModel("prog").getData();
			this._inpsrc = oEvent.getSource();
			if (!this._oEditDialog)
				this._oEditDialog = new CharEditCommon(this._oComponent);
			this._oEditDialog.open(oCharHeader, oInstance, jQuery.proxy(this._onCharEditStore, this, oCharHeader, iIndex, oCollection, this._oEvent1));
		};
	...
	PropInstance.prototype._onAddCountry = function (oEvent) {
		var editusage = new AddCountry(this._oComponent, this._treenode, this._udtmod, this._oldudt, this._cmbbxmod);

	};

	PropInstance.prototype._onCharEditStore = function (oCharHeader, iInstanceIndex, oCollection, oEvent1, aCharValues, bNoUpdate) { // FIXMEVC: refactor
		oCollection.setFieldValue(iInstanceIndex, oCharHeader.getFieldName(), aCharValues, bNoUpdate);
		// this._inpsrc.setValue("");
		var multiinput = this._inpsrc.getModel("prog").getData().multiinput;
		if (multiinput)
			this._inpsrc.removeAllTokens();
		var inpval = aCharValues;

		var sBindingPath = this._inpsrc.getModel("prog").getData().valbindpath;
		var bindpathmf = "/" + iInstanceIndex + "/Mandatoryfields";
		var mandfarr = this._inpsrc.getModel().getProperty(bindpathmf);
		...
		var finbindpath = "/" + iInstanceIndex + "/_ModelManagerADMIN/origEntry/" + sbindpath;
		var orgval = this._inpsrc.getModel().getProperty(finbindpath);
		var prgindref = this._inpsrc.getModel("prog").getData().progindref;
		var mandtot = this._inpsrc.getModel("prog").getData().mandttot;
		var mandtcont = prgindref.getModel().getProperty("/mandtchngdfieldsno");
		var oldval = this._inpsrc.getModel().getProperty(finbindpath);
		if (multiinput) {
		...
		for (var k = 0; k < mandfarr.length; k++) {
			...
			if (mandfarr[k].Label == bindpath[1]) {
				if (mandfarr[k].changecounter == 0) {
					...
					if (inpval == "") {
						this._inpsrc.setValueState("Error");
						mandtcont--;
					} else if (orgval == "" && inpval != "") {
						this._inpsrc.setValueState("None");
						mandtcont++;
					}
				} else {
					if (mandfarr[k].changevalues[mandfarr[k].changevalues.length - 1] == "") {
