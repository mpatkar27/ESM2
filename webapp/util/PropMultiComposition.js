...
sap.ui.define([
	...
	"ESM2/util/Multicompf4"
], function (JSONModel, PropBase, MultilevelAdd, ModelManagerAdmin, ModelManagerAutoGrowOp,
	ModelManagerIntStatus, MessageToast, assert, CharEditCommon, Multicompf4) {
	"use strict";

	var PropMultiComposition = function (oComponent, oTreeNode, oCollection, bEditable, udtprop, speckey, estcat, compindref,
		mandatoryfieldcounter, secndrn, objlay, proplist, oPropInfo, propindex) {
		PropBase.call(this, oComponent, "ESM2.frag.PropMultiComposition");

		this._oComponent = oComponent;
		this._compindref = compindref;
		this._subsec = this._byId("mlcsubsec");
		this._byId("delmlc").setEnabled(bEditable);
		this._byId("addmlc").setEnabled(bEditable);
		...
		var addchildnode = function (oEvent) {
		...
		for (var j = 0; j < headers.length; j++) {
			...
			if (headers[j].ColId == "COMPAVG") {
				...
				var coltree = new sap.ui.table.Column({
					label: label,
					template: textbx
				});
				treetab.addColumn(coltree);
				var coltree1 = new sap.ui.table.Column({
					id: "CL1",
					label: label1,
					template: textbx1,
					visible: false
				});
				treetab.addColumn(coltree1);
				var coltree2 = new sap.ui.table.Column({
					id: "CL2",
					label: label2,
					template: textbx2,
					visible: false
				});
				treetab.addColumn(coltree2);
				var coltree3 = new sap.ui.table.Column({
					id: "CL3",
					label: label3,
					template: textbx3,
					visible: false
				});
				treetab.addColumn(coltree3);

				this.cl1 = coltree1;
				this.cl2 = coltree2;
				this.cl3 = coltree3;
			} else if (headers[j].ColId == "/GMT/COUNTRY_ORIG") {
				...
				var coltree4 = new sap.ui.table.Column({
					label: label,
					template: textbx
				});
				treetab.addColumn(coltree4);
			}

			if (headers[j].ColId == "COMP_TYPE") {
				var labelbut = new sap.m.Label({
					text: headers[j].ColDescr
				});
				var textbx = new sap.m.Text({
					text: "{NoofChildnodes}",
				...
				var btnadd = new sap.m.Button({
					...
					tooltip: "{i18n>addcn}",
					press: jQuery.proxy(addchildnode, this),
					visible: "{ADDCHDBTN}"
				});
				var flxbx = new sap.m.FlexBox({
					justifyContent: "Start"
				});
				flxbx.addItem(textbx);
				flxbx.addItem(btnadd);
				var coltree5 = new sap.ui.table.Column({
					label: labelbut,
					template: flxbx
				});
				treetab.addColumn(coltree5);
