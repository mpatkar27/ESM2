sap.ui.define([
	...
	var ValueHelperDialog = function(oComponent, oValueHelperKey, oInputParams, fOnConfirm) {
		...
		this._oDialog = this._oComponent.getNavigator().createFragment("ESM2.frag.ValueHelperDialog", this);
	...
	ValueHelperDialog.openDialog = function(oComponent, sEntityName, oPropMaps, oParamMaps, oControl, oCollection, fOnSelect) {//FIXME:refactor
		...
		if (oCollection) {
			...
			sFieldName = aPathComps[1];
		}
		
		if(sFieldName == "IDTYPE_NAME")
		sFieldName = "IDTYPE";
		
		if(sFieldName == "IDCAT_NAME")
		sFieldName = "IDCAT";
		...
		var fOnConfirm = function(oEntry) {
			...
			if (oCollection) {
				...
				for (var i = 0; i < aValues.length; i++) {
					var oValue = aValues[i];
					oModel.setProperty("/" + oValue.fieldName, oValue.value);
				}
			}
